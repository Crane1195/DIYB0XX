...
- Multimeter
    - If you are not using an OEM GCC cable, to be safe, you will need to check which wire is which signal. I will post what my wires are below, so if you use a Kitsch Bent extension, they will likely be the same, but not necessarily. It is possible that they change their supplier which changes the wiring.
